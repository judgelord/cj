...
cj_orgs <- cj_comments %>% 
  ...
  filter(!organization %in% c("NA", "Individual", "individual")) %>% 
   mutate(org_name = organization %>%
            str_rm_all("Mass Comment Campaign Sponsored By ") %>%
            clean_orgs() %>%
            str_rm_all("Mass Comment Campaign Sponsored By ")) %>% 
  group_by(org_name) %>% 
...
```{r cj-m-PR-agency-top, fig.width=5, fig.height=3.5}
# cj-m-PR-agency-top
...
```{r cj-m-PR-agency-toptop, fig.height=3, , fig.width=5}
